...
v2.6.0 is this fork's first release. One thing in it has now been confirmed against a real game on real hardware: vibrance applying when a game takes focus, and returning to your normal level when it exits. That is one session, on one machine, by me. Nothing else has been confirmed that way - that session involved no resolution change and had the colour settings switched off, so those paths are still unexercised. The fixes are otherwise backed by 338 automated checks, but those drive fakes and stubs, not a real GPU driver, display or game. Several of them were validated by reading the code and reasoning about documented driver behaviour, not by reproducing the original bug on the reporter's hardware. If you are on a hybrid NVIDIA + AMD laptop or a Thunderbolt eGPU, you are on the least-tested path here. Reports either way are welcome.
